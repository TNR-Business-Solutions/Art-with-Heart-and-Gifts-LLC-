...
class EmailService {
  constructor() {
    this.emailUser = process.env.EMAIL_USER || 'artwithheartandgiftsllc@gmail.com';
  ...
  async sendCommissionInquiry(formData) {
    try {
      const { name, email, phone, projectType, budget, timeline, description } = formData;
...
Sent from Art with Heart & Gifts website
      ...
      // Try Gmail API first
      if (this.gmail) {
        await this.sendViaGmailAPI(emailContent, `New Commission Inquiry from ${name}`, email);
      } else {
...
Sent from Art with Heart & Gifts website
      ...
      // Try Gmail API first
      if (this.gmail) {
        await this.sendViaGmailAPI(emailContent, `Contact Form: ${subject || 'New Message'}`, email);
      } else {
        // Fallback: Use webhook to external service
  ...
  async sendViaWebhook(type, data) {
    ...
    const templateId = type === 'commission' ? 'template_commission' : 'template_contact';
  ...
  async sendTestEmail() {
    try {
      const content = 'This is a test email from the Art with Heart & Gifts website to verify email functionality.';
      
      if (this.gmail) {
        await this.sendViaGmailAPI(content, 'Test Email from Art with Heart & Gifts', this.emailUser);
      } else {
...
Sent from Art with Heart & Gifts website
      ...
      if (this.gmail) {
        await this.sendViaGmailAPI(emailContent, `New Order - ${customerInfo.name}`, customerInfo.email);
      } else {
