...
class EmailService {
  constructor() {
    ...
    if (!this.apiKey) {
      ...
      console.warn('📧 Set SENDGRID_API_KEY environment variable to enable emails.');
    } else {
  ...
  async sendCommissionInquiry(formData) {
    if (!this.apiKey) {
      return {
        success: false,
        error: 'Email service not configured. Please contact us directly at artwithheartandgifts@yahoo.com',
      };
    }
...
Sent from Art with Heart & Gifts website
      ...
      const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
        ...
        body: JSON.stringify({
          ...
          from: { email: this.fromEmail, name: 'Art with Heart & Gifts Website' },
      ...
      if (response.ok) {
        ...
        return { success: true, message: 'Commission inquiry sent successfully!' };
      } else {
      ...
      const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
        ...
        body: JSON.stringify({
          ...
          content: [
            {
              type: 'text/plain',
              value: 'This is a test email from the Art with Heart & Gifts website to verify email functionality.'
            }
          ]
        })
      });
