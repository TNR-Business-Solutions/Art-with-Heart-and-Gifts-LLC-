...
const EmailService = require("./email-service-gmail-fixed");
  ...
  try {
    ...
    if (!name || !email || !description) {
      return res.status(400).json({
        success: false,
        error:
          "Missing required fields: name, email, and project description are required",
      });
    }
    ...
    if (result.success) {
      res.json({
        success: true,
        message:
          "Your commission inquiry has been sent successfully! We'll get back to you within 24 hours to discuss your project.",
        messageId: result.messageId,
      });
    } else {
      res.status(500).json({
